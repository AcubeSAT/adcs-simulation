...
%         Control_Thomson(x(5:7), R_OB * mag_field_orbit2 * 10^(-9)+ norm(sqrt(R)) * randn(size( y_real(1:3) * norm(mag_field_orbit(:, current_timestep-1)*10^(-9)))), y_real(1:3) * norm(mag_field_orbit(:, current_timestep-1)*10^(-9)), Const.mtq_max);
% 
% 
% 
%   %% Angle between Z-Axis of b.f. and Z-Axis of o.f.
