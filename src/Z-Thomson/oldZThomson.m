...
% Define face colors (cyan for all, magenta for +Y)
face_colors = repmat([0, 1, 1], size(faces, 1), 1); % Default cyan for all faces
face_colors(5, :) = [1, 0, 1];                     % Magenta for +Y face (Face 5)
...
% Create patch object for the rectangular prism with face-specific colors
...
quiver_handle = quiver3(0, 0, 0, 0, 0, 0, 'r', 'LineWidth', 2, 'MaxHeadSize', 1);
...
for t = 1:length(Time)
    ...
    % Extract B_body vector (already in the orbit frame)
    scale_factor = 1e6;
    B_orbit = B_body(:, t);  % No transformation needed
    B_orbit = R_OB' * B_orbit;
    B_orbit = B_orbit * scale_factor;

    set(quiver_handle, ...
        'XData', 0, 'YData', 0, 'ZData', 0, ...               % Origin
        'UData', B_orbit(1), 'VData', B_orbit(2), 'WData', B_orbit(3)); % Vector components
