%% ========================================================================
% Wahba's Problem:
% This function solves the Wahba's Problem using the SVD Algorithm. The
...
function [quat_r, Ropt_r] = wahba(sun_body, mgn_body, sun_ref, mgn_ref)
    ...
    for i = 1:3
        A(i, 1) = sun_weight * sun_body(1) * sun_ref_vec(i) + mgn_weight * mgn_body(1) * mgn_ref_vec(i);
        A(i, 2) = sun_weight * sun_body(2) * sun_ref_vec(i) + mgn_weight * mgn_body(2) * mgn_ref_vec(i);
        A(i, 3) = sun_weight * sun_body(3) * sun_ref_vec(i) + mgn_weight * mgn_body(3) * mgn_ref_vec(i);
    end
