function peaks=findPeaks(datas,thresh,iSubset,varargin)
%        peaks=findPeaks(datas,thresh,iSubset,minHits,dXmax,dYmax)
%  Locate peak clusters greater>thresh
% 
%  data      = nyXnx  data array  
%  threshold = in data units
%  iSubset   = indices of points included []=> full set 
%
%  minHits   = minimum # of hits for cluster
%  dXmax, dYmax = max extent of cluster coordinates
...
while nPeaks2test > minHits
    ...
    if length(iHits)>minHits
        ...
        peaks(iCount).area     = length(peakVals);
    end;
