...
mekf = MEKF(n_params, n_msr, @model.stateTransFun, @model.msrFun); %Init MEKF
...
%% ============ IdealQ ============================================        
        ...
        gyro = y_noise(4:6);
        mekf.correct(y_noise, msrCookieFinalExtended(mag_field_eci(:,(k-1)/dt+c),...
        ...
        [torq, T_rw, T_magnetic_effective, V_rw, I_rw, P_thermal_rw, AngVel_rw_rpm_next, AngVel_rw_radps_next,...
                acceleration_rw_cur, rw_ang_momentum, init_AngVel_dz, init_accel_dz, V_mtq, I_mtq, P_thermal_mtq, ...
                    timeflag_dz,q_sun_body] = ...
                        PD_Sun_Pointing(q_desired, x_hat(1:4), y_noise(4:6)-mekf.global_state(5:7) , y_noise(1:3)*norm(mag_field_orbit(:,(k-1)/dt+c)*10^(-9)), ...
                            Const.mtq_max, Const.lim_dz, AngVel_rw_radps(2,1), AngVel_rw_rpm(2,1), ...
                                acceleration_rw(1,1), init_AngVel_dz, init_accel_dz, timeflag_dz,Const.rw_max_torque,y_real(1:3)*norm(mag_field_orbit(:,(k-1)/dt+c)*10^(-9)), l,sun_pos_eci(:,(k-1)/dt+c), Const.known_rm);
