...
Const=constants();
setParamsFinal_Nominal_Moustakas_Pappa();
eclipse_data=0;
...
for l=1:n_steps
	...
	gyro_noise_data = [gyro_noise_data gyro_noise];
        ...
        y_noise(1:3)=y_noise(1:3)/norm(y_noise(1:3)); 
        qd = [qd [0 (sun_pos_orbit(:,(k-1)/dt+c)/norm(sun_pos_orbit(:,(k-1)/dt+c)))']'];
        gyro = y_noise(4:6);
        ...
        [torq, T_rw, T_magnetic_effective, V_rw, I_rw, P_thermal_rw, AngVel_rw_rpm_next, AngVel_rw_radps_next,...
                acceleration_rw_cur, rw_ang_momentum, init_AngVel_dz, init_accel_dz, V_mtq, I_mtq, P_thermal_mtq, ...
                    timeflag_dz,q_desired] = ...
                        PD_Moustakas_Pappa(q_desired ,q_ob_hat, y_noise(4:6)-ekf.theta(5:7) , y_noise(1:3)*norm(mag_field_orbit(:,(k-1)/dt+c)*10^(-9)) , eclipse((k-1)/dt+c), ...
                            Const.mtq_max, Const.lim_dz, AngVel_rw_radps(2,1), AngVel_rw_rpm(2,1), ...
                                acceleration_rw(1,1), init_AngVel_dz, init_accel_dz, timeflag_dz,Const.rw_max_torque,y_real(1:3)*norm(mag_field_orbit(:,(k-1)/dt+c)*10^(-9)), l,sun_pos_orbit(:,(k-1)/dt+c));
        qd = [qd (q_desired/norm(q_desired))'];
...
x_real_euler_perf = quat2eul(q_ob_data(1:4,21:length(q_ob_data)-1)'); 
x_real_euler_perf = rad2deg(x_real_euler_perf');
x_desired = quat2eul(qd(1:4,1:length(qd))');
